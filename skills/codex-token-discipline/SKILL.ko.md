...
- 큰 tool result는 다음 turn의 입력 비용으로 본다. 1만 자 안팎을 넘길 가능성이 있는 명령은 전체 출력 전에 count, path, summary, 첫 actionable failure를 먼저 요청한다.
...
스크립트는 Codex rollout log를 읽고 subagent/fork를 root thread로 묶어 진단한다. raw payload를 출력하지 않고 token total, tool-output size, large-output event, browser/image signal을 보여준다. 토큰 총량은 품질 기준이 아니라 신호로 본다.

감사 중 홈 전체 텍스트 검색은 피하고, `$CODEX_HOME/sessions` 같은 명시적 sessions root를 지정한다.
