...
- Applied post-release review compaction to `SKILL.md` and `SKILL.ko.md`: shortened the trigger description, long-running-work section, subagent guidance, and usage-audit section without changing the README/docs routing.
- Validated the compacted skill with diff whitespace check, project-context runtime checker, skill-creator quick validator, syntax parse, and usage-script help. Pushed commit `96b9a57` and refreshed the global install with `npx skills update codex-token-discipline -g -y`.
